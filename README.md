...
Each training run now streams detailed metrics (train/validation losses, running averages, gradient norms, learning rate, wall-clock timings, and token counts) into timestamped JSON files under `metrics/train_metrics_<model>_<run_id>.json`, flushing updates every 100 optimization steps. Run `python plot.py` after training to overlay BDH and Transformer curves; separate train and validation PNGs (`..._train.png`, `..._val.png`) are written to `plots/` by default. The Transformer baseline widens to 8 layers × 312-d embeddings with a ×14 MLP—yielding ~25.3 M parameters, within 0.003% of BDH’s count—for apples-to-apples comparisons.
